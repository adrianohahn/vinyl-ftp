...
module.exports = {
	...
	upload: function ( file, path, cb, mode ) {
	...
	downbuffer: function ( path, cb, mode ) {
	...
	downstream: function ( path, cb, mode ) {
	...
	acquire: function ( cb, mode ) {
		var self = this;
		mode = mode || 'bin';

		if ( this.idle.length > 0 ) {

			onAcquire( null, this.idle.shift() );
	...
	setMode: function (mode, ftp, cb) {
		...
		if (mode === 'ascii') {
			...
			self.log( 'BIN  ');
			ftp.binary(onModeSet);

		}
			
		function onModeSet(err) {
			ftp.mode = mode;
			self.log( 'MSET ', mode + ' Done');
			cb(err, ftp);
		} 
	},
